...
def bn_act_conv_drp(current, num_outputs, kernel_size, scope='block'): #composition function
...
def block(net, layers, growth, scope='block'): # bottleneck layer (internal layer)
...
def densenet(images, num_classes=1001, is_training=False,
    ...
    growth = 24
    compression_rate = 0.5#提高模型的紧凑性，θ=0.5,当使用θ<1的瓶颈和过渡层，为Densenet-BC

    def reduce_dim(input_feature):
        return int(int(input_feature.shape[-1]) * compression_rate)#channels
    ...
    with tf.variable_scope(scope, 'DenseNet', [images, num_classes]):
        with slim.arg_scope(bn_drp_scope(is_training=is_training,
            ...
            pass
            scope = "conv1" 
            net = slim.conv2d(images, 2 * growth, [7, 7], stride=2, scope=scope)#48
